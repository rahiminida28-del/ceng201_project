public class ReportService {
    ...
    private static void swap(Submission[] heap, int first, int second){
        ...
        heap[first]=heap[second];   //burda swap metodu arraydaki 2 elamanin yerlerini birbryle degistirmk (takas etmk ) icin kullanildi
        heap[second] = temp;
    }
    ...
    private static void siftUp(Submission[] heap, int index){
        ...
        while (index>0) {
            ...
            int parent = (index-1)/2;                //find the  index from  parent.

            if (heap[index].getSizeKb() < heap[parent].getSizeKb()) {  //child parent kucukse yerlerini degistir (min heap kurali)

                swap(heap, index, parent);

                index = parent; //takip edilen index yukari parent tasi
            } else {

                break; //if its true then stop it
            }
        }
    }
     //moved the element down to maintain min heap property
    private static void siftDown(Submission[] heap, int size, int index){
        while (true){
            int left = 2*index+1;   //left child index
            int right = 2*index+2;  //right child index
            int smallest = index;   //baslangicta en kck olarak mevcut node sec
    ...
    public static Submission[] topKLargest(Submission[] all, int k){
        ...
        for (int i = k; i < all.length ; i++) {
            ...
            if (all[i].getSizeKb() > heap[0].getSizeKb()) {  //En byk k elemani tut: yeni eleman kokten bykse koke koy ve agaci duzelt
    ...
    //in Insertion Sort we use timestampMS
    ...
    public Submission[] sortByTimeInsertion(Submission[] all){
        ...
        for (int i = 1; i < result.length ; i++) {    //2.elemandan baslayark her elemani soldaki sirali bolume yerlestir
    ...
    private  Submission[] merge(Submission[] array1, Submission[] array2){  //sirali 2 diziyi tek bir sirali dizi olarak birlestir
    ...
    private Submission [] mergeSort(Submission[] array){
        ...
        if (array.length <=1) {                       //Base Case already sorted also  0 or 1 elemant sorted
            return array;
        }
