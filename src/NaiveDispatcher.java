public class NaiveDispatcher {
    ...
    public NaiveDispatcher(int capacity){
     ...
     //iki gonderiden hangisinin daha oncelikli oldugunu belirlemesi icin
    public boolean highPriority(Submission first, Submission second){

        //eger ilk ogrencinin ozel durumu varsa
        //ikincinin yoksa ilk olana oncelik verir
        if (first.hasAccommodation() && !second.hasAccommodation()) {
            return true;
        }
        //ilkin tam tersi mesala ilk ogrencin ozel durmu yok ama
        //ikincinin varsa ilk olana oncelik verme
        if (!first.hasAccommodation() && second.hasAccommodation()) {
            return false;
        }
         //ikisinin durumu da esitse odevi ilk yukleyene oncelik verir
